...
case "$GATE" in
  transport)
    ...
    ;;
  boundaries)
    echo "boundary gate requires a live MAPI instance:" >&2
    echo "  run-gate-boundaries.sh <base-url> <token-env-name>" >&2
    echo "  (measures safe-boundary dispatch latency; spec §18)" >&2
    exit 2
    ;;
  overhead)
    echo "overhead gate requires game runs with the mod disabled/enabled:" >&2
    echo "  run-gate-overhead.sh" >&2
    echo "  (tick/frame benchmark; spec §18)" >&2
    exit 2
    ;;
  reliability|leaks|parallel|smoke|security|release-jars|sdk-interop)
